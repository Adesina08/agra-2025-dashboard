...
type Normalizer = (row: SheetRow, index: number) => FarmerData | EnterpriseData | YouthData;

const normalizerMap: Record<SurveyKey, Normalizer> = {
...
export function useSurveySheet<T extends FarmerData | EnterpriseData | YouthData>(survey: SurveyKey): SurveyResult<T> {
  ...
  // Fallback to empty data if live sheet fails or is empty
